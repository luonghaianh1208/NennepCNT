...
<div class="content">
  ...
  ${['Nhập liệu vi phạm/thành tích tức thì qua trình duyệt trên điện thoại hoặc máy tính — không cần cài đặt','Hệ thống tự động tính điểm theo công thức chuẩn, cập nhật xếp hạng lớp theo tuần/tháng/học kỳ','Dữ liệu tuần đang diễn ra đồng bộ trực tiếp: một người nhập, mọi máy đang mở thấy ngay','Giao diện tra cứu mở cho học sinh và phụ huynh, không cần đăng nhập','Xuất báo cáo DOCX/Excel chỉ một cái bấm, đúng định dạng nhà trường yêu cầu','Dữ liệu và ảnh minh chứng lưu trên hạ tầng đám mây tiêu chuẩn quốc tế — bền vững, sao lưu định kỳ, xuất ra bất kỳ lúc nào'].map(t=>`<div class="bullet">${t}</div>`).join('')}
...
<div class="content">
  ...
  ${[['1','⚙️ Admin cấu hình hệ thống','Thiết lập danh sách lớp, học sinh, tiêu chí vi phạm/thành tích, điểm quy đổi và phân kỳ thời gian. Thao tác trực tiếp trên giao diện Settings — không cần chỉnh dữ liệu thô.'],['2','📝 Giáo viên / Ban nề nếp nhập liệu','Mỗi ngày, cán bộ được phân quyền nhập vi phạm hoặc thành tích. Có thể nhập từng dòng, nhập nhiều lớp cùng lúc, hoặc nhập hàng loạt từ file Excel theo mẫu. Ảnh minh chứng đính kèm trực tiếp.'],['3','🤖 Hệ thống tự động tính toán','Điểm thi đua mỗi lớp tính lại tự động: Điểm = 500 × (số tuần trong kỳ) − tổng điểm trừ + tổng điểm cộng. Không cần thao tác thủ công.'],['4','📊 Xem xếp hạng & tra cứu','Giáo viên, học sinh, phụ huynh tra cứu bảng xếp hạng và lịch sử vi phạm của từng lớp — không cần đăng nhập.'],['5','📋 Xuất báo cáo cuối kỳ','Admin một cái bấm để xuất DOCX (báo cáo chi tiết) hoặc Excel (bảng tổng hợp điểm). Đúng chuẩn, sẵn sàng nộp Ban Giám Hiệu.']].map(([n,title,desc],i,arr)=>`<div class="step"><div class="step-num">${n}</div><div class="step-body"><h4>${title}</h4><p>${desc}</p></div></div>${i<arr.length-1?'<div class="step-arrow">↓</div>':''}`).join('')}
</div>
...
<div class="content">
  ...
  ${['Mỗi tài khoản được gán 1 vai trò cố định, quản trị viên có thể điều chỉnh bất kỳ lúc nào.','Tài khoản Khách không cần đăng nhập — thiết kế có chủ ý để minh bạch thông tin toàn trường.','Mật khẩu được kiểm tra và mã hoá phía hệ thống — máy của người dùng không bao giờ lưu mật khẩu gốc.','Nhật ký xóa (Audit Log) ghi lại đầy đủ ai xóa gì, lúc nào — hỗ trợ kiểm tra nội bộ.'].map(t=>`<div class="bullet">${t}</div>`).join('')}
  <div class="card" style="margin-top:20px"><div style="color:#CF2A2A;font-weight:700;font-size:13px;margin-bottom:6px">🔒 Bảo mật dữ liệu</div><p style="font-size:12px;color:#444;line-height:1.7">Mật khẩu được mã hoá một chiều theo chuẩn quốc tế — kể cả quản trị viên hệ thống cũng không đọc được mật khẩu của bất kỳ ai; khi quên thì đặt lại qua thư điện tử chứ không ai cấp lại được. Quyền ghi dữ liệu được kiểm soát ở tầng lưu trữ chứ không chỉ ẩn nút trên giao diện, nên không thể lách bằng thao tác kỹ thuật. Mọi thao tác quản trị đều lưu vào Nhật ký thao tác để đối chiếu khi cần.</p></div>
</div>
...
<div class="content">
  ...
  <div style="background:linear-gradient(135deg,#e8f5e9,#f1f8e9);border:2px solid #4caf50;border-radius:10px;padding:20px;text-align:center;margin:10px 0 18px">
    <div style="font-size:22px;font-weight:900;color:#2e7d32">✅ NHÀ TRƯỜNG KHÔNG PHẢI LO HẠ TẦNG</div>
    <div style="font-size:12px;color:#555;margin-top:8px">Toàn bộ chi phí máy chủ, đường truyền và lưu trữ do đơn vị cung cấp chịu trách nhiệm — nhà trường không phải mua sắm thiết bị, không phải thuê máy chủ, không cần nhân sự kỹ thuật vận hành</div>
  </div>
  <table><thead><tr><th>Thành phần</th><th>Mô tả</th><th style="width:120px;text-align:center">Chi phí</th></tr></thead><tbody>
    ${[['Hệ thống dữ liệu riêng','Mỗi trường một kho dữ liệu độc lập, không dùng chung với trường khác'],['Đăng nhập & phân quyền','Tài khoản theo vai trò, đặt lại mật khẩu qua thư điện tử'],['Kho ảnh minh chứng','Lưu trữ và nén ảnh tự động, truy cập nhanh trên điện thoại'],['Đường truyền & tên miền','Địa chỉ web riêng cho nhà trường, kết nối mã hoá'],['Sao lưu & giám sát','Sao lưu định kỳ, theo dõi hoạt động hệ thống']].map(([n,d])=>`<tr><td style="font-weight:700">${n}</td><td>${d}</td><td class="td-grn" style="text-align:center">Đơn vị cung cấp chịu</td></tr>`).join('')}
  </tbody></table>
  ...
  <div class="section-hdr" style="margin-top:20px"><span>💵</span> PHÍ DUY TRÌ HỆ THỐNG</div>
  <table><thead><tr><th>Hình thức</th><th>Ưu đãi</th><th style="width:190px;text-align:center">Phí duy trì quy đổi</th></tr></thead><tbody>
    <tr><td style="font-weight:700">Ký theo từng năm</td><td>Mức niêm yết</td><td style="text-align:center">500.000 – 620.000đ / tháng</td></tr>
    <tr><td style="font-weight:700;color:#CF2A2A">Đặt hàng sớm</td><td>Chiết khấu <strong>15–20%</strong> cho đơn vị đăng ký trong đợt đầu</td><td class="td-grn" style="text-align:center">420.000 – 500.000đ / tháng</td></tr>
    <tr><td style="font-weight:700;color:#1E459F">Ký nhiều năm (từ 3 năm)</td><td>Chiết khấu <strong>tới 40%</strong>, khoá giá trong suốt thời hạn hợp đồng</td><td class="td-grn" style="text-align:center">300.000 – 400.000đ / tháng</td></tr>
  </tbody></table>
  <div class="card"><div style="color:#1E459F;font-weight:700;font-size:13px;margin-bottom:6px">💡 Lưu ý về đầu tư</div><p style="font-size:12px;color:#444;line-height:1.7">Mức niêm yết tương đương <strong>20–25 USD/tháng</strong>. Phí duy trì đã bao gồm toàn bộ chi phí hạ tầng (do đơn vị cung cấp chi trả), sao lưu, cập nhật phiên bản và hỗ trợ kỹ thuật trong suốt thời hạn hợp đồng — nhà trường không phát sinh thêm khoản nào. Mức phí trên mang tính tham khảo; <strong>giá chính thức được xác định theo hợp đồng ký kết giữa hai bên</strong>, tuỳ quy mô nhà trường và phạm vi triển khai.</p></div>
</div>
...
<div class="content">
  ...
  <table><thead><tr><th style="width:60px;text-align:center">Bước</th><th style="width:100px;text-align:center">Thời gian</th><th>Nội dung</th></tr></thead><tbody>
    ${[['1','1–2 ngày','Khảo sát nhu cầu: cơ cấu lớp, tiêu chí nền nếp, mẫu báo cáo, danh sách tài khoản'],['2','1–2 ngày','Dựng hệ thống riêng cho nhà trường: địa chỉ web riêng, gắn logo và tên trường'],['3','1 ngày','Nhập dữ liệu nền: danh sách lớp, học sinh, tiêu chí, phân kỳ thời gian'],['4','1 buổi','Đào tạo sử dụng cho cán bộ quản lý và giáo viên nhập liệu'],['5','7–30 ngày','Vận hành thử — hỗ trợ qua điện thoại/Zalo, điều chỉnh theo phản hồi'],['6','Ổn định','Bàn giao chính thức, cam kết hỗ trợ kỹ thuật theo gói đã chọn']].map(([n,t,c])=>`<tr><td style="text-align:center;font-weight:900;color:white;background:linear-gradient(135deg,#1E459F,#CF2A2A)">${n}</td><td style="text-align:center;color:#CF2A2A;font-weight:700">${t}</td><td>${c}</td></tr>`).join('')}
  </tbody></table>
  <div class="sub-hdr">⚙️ Yêu cầu tối thiểu</div>
  ${['Danh sách lớp, học sinh và bộ tiêu chí nền nếp của nhà trường','Địa chỉ thư điện tử cho các tài khoản sử dụng','Kết nối Internet ổn định (để đồng bộ dữ liệu khi nhập liệu)','Thiết bị nhập liệu: smartphone hoặc máy tính (không cần cài đặt gì)','Ít nhất 1 người phụ trách kỹ thuật tại trường để phối hợp cài đặt'].map(t=>`<div class="bullet">${t}</div>`).join('')}
...
<div class="last-page">
  ...
  <div class="commit-box">
    ...
    <div class="commit-item">Phiên bản 4.0 (tháng 8/2026): nâng cấp toàn diện nền tảng — đồng bộ trực tiếp theo thời gian thực, đăng nhập bảo mật chuẩn quốc tế, ảnh minh chứng tự nén, mở ứng dụng nhanh gấp nhiều lần</div>
