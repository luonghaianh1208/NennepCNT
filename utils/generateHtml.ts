...
<div class="content">
  ...
  ${[['📝','Nhập liệu vi phạm & thành tích','Nhập nhanh qua form gợi ý; hỗ trợ vi phạm cá nhân và tập thể; đính kèm ảnh minh chứng; import hàng loạt qua CSV.'],['🔍','Tra cứu & Lọc nâng cao','Lọc theo lớp, tiêu chí, loại, khoảng thời gian. Nổi bật bản ghi trùng lặp để kiểm duyệt.'],['🏆','Xếp hạng tự động theo kỳ','Bảng xếp hạng cập nhật theo tuần/tháng/học kỳ; phân theo khối; màu thể hiện thứ hạng.'],['📄','Xuất báo cáo DOCX','Tự động tạo file Word đúng mẫu báo cáo; đầy đủ vi phạm cá nhân và tập thể theo tiêu chí.'],['📊','Xuất bảng Excel','File Excel tự tính điểm tổng, sẵn sàng nộp cho Ban Giám Hiệu hoặc lưu trữ.'],['⚙️','Cấu hình linh hoạt','Admin cấu hình tiêu chí, điểm, thời gian, tài khoản và phân quyền trực tiếp trên hệ thống.'],['🔐','Bảo mật đăng nhập','Xác thực server-side; mật khẩu không bao giờ rời server. Phiên đăng nhập bền vững.'],['✏️','Sửa hàng loạt (Bulk Edit)','Chọn nhiều vi phạm → đổi ngày/ghi chú/tiêu chí chỉ trong 1 thao tác. Hoàn tác trong 8 giây.'],['⚠️','Cảnh báo thông minh','Cảnh báo vi phạm ngoài kỳ, trùng tuần; badge nổi bật bản ghi lệch kỳ cấu hình.'],['📱','Tối ưu mobile','Giao diện responsive, hoạt động mượt mà trên iOS/Android — không cần cài app.'],['🎨','Đa giao diện','Chế độ Đoàn TNCS (đỏ-vàng) và Winter (xanh tuyết) — đổi trực tiếp trên header.']].map(([ico,title,desc])=>`<div style="display:flex;gap:12px;margin-bottom:12px;align-items:flex-start"><div style="min-width:34px;height:34px;background:linear-gradient(135deg,#1E459F,#CF2A2A);border-radius:8px;display:flex;align-items:center;justify-content:center;font-size:18px;flex-shrink:0">${ico}</div><div><div style="color:#1E459F;font-weight:700;font-size:13px;margin-bottom:2px">${title}</div><div style="color:#444;font-size:12px;line-height:1.6">${desc}</div></div></div>`).join('')}
</div>
